...
Shaders* ShadersCreate(Gpu& g)
{
    ...
    ok &= GpuMakeCompute(g, g_cs_compose_residual, sizeof g_cs_compose_residual, 4, 1, 13, s->compose_residual, L"cs_compose_residual");
               ...
               UINT ww, UINT wh, ID3D12Resource* out, UINT w, UINT h, const ComposeParams& p)
{
    ...
    struct { float strength, chroma; UINT wipe_mode; float wipe_x; int feather; UINT nrects, w, h, ww, wh, strip_w, strip_h; } c =
        { p.residual_strength, p.chroma, (UINT)p.wipe_mode, p.wipe_x, p.feather, n, w, h, ww, wh, (UINT)p.strip_w, (UINT)p.strip_h };
...
{
    const UINT n = UploadRects(g, s, cl, p);
    struct { float strength, chroma; UINT wipe_mode; float wipe_x; int feather; UINT nrects, w, h, ww, wh; float warp; UINT strip_w, strip_h; } c =
        { p.residual_strength, p.chroma, (UINT)p.wipe_mode, p.wipe_x, p.feather, n, w, h, ww, wh, p.warp, (UINT)p.strip_w, (UINT)p.strip_h };
...
{
    struct { UINT w, h; float strength, saturation; UINT nrects; } c = { w, h, strength, saturation, std::min(nrects, kMaxRects) };
